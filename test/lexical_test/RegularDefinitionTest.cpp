...
TEST(RegularDefinition, Solver2) {
    string test = "(a|b|c)";
    RegularDefinition::Tokenize(test);
    vector<RegularDefinition *> regular_definition_vector;
    for(int i = 0; i < test.size(); ++i) {
        std::string *pstr = new std::string;
        *pstr = test[i];
    ...
    cout << "Going into solver" << endl;
    // Should implement how to check for the solver function.
    Nfa::Solver(RegularDefinition::Tokenize(test));
}

TEST(RegularDefinition, Solver) {
    string test = "(a*(k|l|(mn|k))*l)+";
    vector<RegularDefinition *> regular_definition_vector;
    for(int i = 0; i < test.size(); ++i) {
        std::string *pstr = new std::string;
    ...
    // Should implement how to check for the solver function.
    Nfa::Solver(regular_definition_vector);
}
